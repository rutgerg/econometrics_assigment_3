...
Cointegration happens when two or more series share the same stochastic trend. In this csae that makes sense since stockprices rise (cp) with rising dividends because it assumes higher future cashflows. Cointegration says that if both series are non-stationary then difference can be stable or stationary and H0 should be reject in favor of H1. 
...
Since Padf-value is 0.23 we do not reject H0 so there is non stationary data and have unit root. There is not enough evidence to support cointegration. IS THIS A CORRECT CONCLUSION?
...
Let's test:
```{r echo=FALSE}
tsdata1c <- ts(MyQ1Data$ret)
VARselect(tsdata1c, lag.max=2, type='const')
```

```{r echo=FALSE}
#var1c <- VAR(tsdata1c, p=2, type='const')
#summary(var1c)
```
FUNCTION ABOVE GIVE ERRROR? IK WIL MATRIX VAN JAAR EN ret MAAR DAT LUKT NIET.
...
The Granger causality test Granger is an F test of the null hypothesis that all lags of a variable X included in a time series regression model do not have predictive power for Yt. The Granger causality test does not test whether X actually causes Y but whether the included lags are informative in terms of predicting Y.
...
ADL(3,2)
```{r echo=FALSE}
aq1d  <- dynlm(ret ~ L(ret,1) + L(ret, 2) + L(ret, 3) + L(TSpr,1) + L(TSpr,2) + L(TSpr,3), data=MyQ1Data)
coeftest(aq1d, vcov. = sandwich)

#linearHypothesis(aq1d, c("L(ret)=0","L(ret,2)=0","L(ret,3)=0","L(TSpr)=0","L(TSpr,2)=0","L(TSpr,3)=0"), vcov. = sandwich)

```

Uncommenting line 166 gives error: "Error in linearHypothesis.lm(aq1d, c("L(ret,3)=0", "L(TSpr)=0"), vcov. = sandwich) : 
  residual sum of squares is 0 (within rounding error)" must be something wrong
