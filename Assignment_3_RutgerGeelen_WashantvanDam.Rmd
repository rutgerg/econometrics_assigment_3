...
ADL(3,2)
```{r echo=FALSE}
aq1d  <- dynlm(ret ~ L(ret, 3) + L(TSpr), data=MyQ1Data)
#coeftest(aq1d, vcov. = sandwich)

#linearHypothesis(aq1d, c("L(ret,3)=0", "L(TSpr)=0"), vcov. = sandwich)
```

Uncommenting line 167 gives error: "Error in linearHypothesis.lm(aq1d, c("L(ret,3)=0", "L(TSpr)=0"), vcov. = sandwich) : 
  residual sum of squares is 0 (within rounding error)" must be soemthing wrong
